...
class Pedal {
  ...
  getPedalOptions(data,callback) {
    ...
    callback({"value":arr,"pedal":data});
  }
  
  getParamOptions(data,callback) {
    let [pedal,param] = data;
    this.effects.map(pdl => {
      if (pdl.id === pedal) {
        for (let i in pdl.param) {
          if (i === param) {
            if (pdl.param[i].menuItems) {
              let arr = [];
              for (let j in pdl.param[i].menuItems) {
                arr.push(j)
              }
              callback(arr);
            }
      ...
      if (pedal.id === data.value) {
        for (let i in pedal.param) {
          if (pedal.param[i].type != "hidden") {
            ...
            if (!menuItems) {
              if (pedal.midi) {
                menuItems = (pedal.msblsb)?{"0 - 255":0}:{"0 - 127":0};
              }
  ...
  turnAllOff(callback) {
    this.effects.map(pedal => {
      let state = pedal.param["state"];
      state = state?state.value:0;
      if (state > 0) {
        let returndata = {"pedal":pedal.number,"id":pedal.id,"param":pedal.param["state"].number,"value":0,"midi":pedal.midi?1:0,"ramp":0};
        callback(returndata);
      }
    });
  }
